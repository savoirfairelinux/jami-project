...
DNF_CLIENT_QT_DEPENDENCIES = [
    'qt5-qtsvg-devel', 'qt5-qtwebengine-devel', 'qt5-multimedia-devel', 'qt5-qtdeclarative-devel'
]
...
def run_dependencies(args):
    ...
    if args.distribution == WIN32_DISTRIBUTION_NAME:
        ...
        if parsed_args.qt is None:
            APT_DEPENDENCIES.append(APT_CLIENT_GNOME_DEPENDENCIES)
        else:
            APT_DEPENDENCIES.append(APT_CLIENT_QT_DEPENDENCIES)
        execute_script(
            APT_INSTALL_SCRIPT,
        ...
        if parsed_args.qt is None:
            DNF_DEPENDENCIES.append(DNF_CLIENT_GNOME_DEPENDENCIES)
        else:
            DNF_DEPENDENCIES.append(DNF_CLIENT_QT_DEPENDENCIES)
        execute_script(
            RPM_INSTALL_SCRIPT,
        ...
        if parsed_args.qt is None:
            ZYPPER_DEPENDENCIES.append(ZYPPER_CLIENT_GNOME_DEPENDENCIES)
        else:
            ZYPPER_DEPENDENCIES.append(ZYPPER_CLIENT_QT_DEPENDENCIES)
        execute_script(
            ZYPPER_INSTALL_SCRIPT,
