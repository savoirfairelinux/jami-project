...
def run_install(args):
    ...
    if args.distribution == "OSX":
        ...
        execute_script(["CONFIGURE_FLAGS='--without-dbus " + os.environ["RING_DAEMON_CONF"] + "' ./scripts/install.sh " + install_args])
    elif args.distribution == "Android":
        os.chdir("./client-android")
        execute_script(["./compile.sh"])
    else:
        install_args += ' -c client-gnome'
        execute_script(["CONFIGURE_FLAGS=" + os.environ["RING_DAEMON_CONF"] + "' ./scripts/install.sh " + install_args])
