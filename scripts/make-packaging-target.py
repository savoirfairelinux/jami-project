...
packages/%(distribution)s/$(DEBIAN_AMD64_CHANGES_FILENAME): docker-image-%(distribution)s release-tarball packages/%(distribution)s
	$(PACKAGE_%(distribution)s_DOCKER_RUN_COMMAND)

.PHONY: package-%(distribution)s
package-%(distribution)s: packages/%(distribution)s/$(DEBIAN_AMD64_CHANGES_FILENAME)
