...
if [ ! -f "${RPM_PATH}" ]; then
    ...
    (
        ...
        if [[ "${DISTRIBUTION:0:4}" == "rhel" ]]; then
            ...
            mv "/root/rpmbuild/RPMS/x86_64/jami-libqt-$QT_MAJOR_MINOR_PATCH-1.fc33.x86_64.rpm" "${RPM_PATH}"
        elif [[ "${DISTRIBUTION}" == "fedora_34" ]]; then
            mv "/root/rpmbuild/RPMS/x86_64/jami-libqt-$QT_MAJOR_MINOR_PATCH-1.fc34.x86_64.rpm" "${RPM_PATH}"
        else
