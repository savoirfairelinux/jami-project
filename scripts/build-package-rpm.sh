...
tar xf $RELEASE_TARBALL_FILENAME ring-project/packaging/rules/rpm \
...
cp --reflink=auto "$RELEASE_TARBALL_FILENAME" /root/rpmbuild/SOURCES
...
if [[ "${DISTRIBUTION:0:4}" == "rhel" \
   || "${DISTRIBUTION:0:13}" == "opensuse-leap" ]]; then
    ...
    if [ ! -f "${RPM_PATH}" ]; then
        ...
        tar xf "$RELEASE_TARBALL_FILENAME" \
