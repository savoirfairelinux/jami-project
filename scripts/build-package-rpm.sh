...
if [[ "${DISTRIBUTION}" != "opensuse-tumbleweed" ]]; then
    ...
    if [ ! -f "${RPM_PATH}" ]; then
        ...
        (
            ...
            QA_CHECK_RPATHS=0 rpmbuild -ba jami-libqt.spec
            mkdir -p "$TARBALLS/${DISTRIBUTION}"
