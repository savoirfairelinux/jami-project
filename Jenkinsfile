    ...
    stages {
        ...
        stage('Generate release tarball') {
            steps {
                ...
                // environment.
                sh '''
                   guix environment --pure --expose=TARBALLS --ad-hoc \
                       coreutils \
                       gcc-toolchain \
                       git-minimal \
                       grep \
        ...
        stage('Build packages') {
            ...
            parallel{
                stage('Build package-ubuntu_20.04') {
                    steps {
                        sh "make package-ubuntu_20.04"
                    }
                }

                stage('Build package-debian_10') {
                    steps {
                        sh "make package-debian_10"
                    }
                }
            }
        }
