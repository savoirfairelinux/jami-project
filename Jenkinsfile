...
pipeline {
    ...
    stages {
        ...
        stage('Fetch contrib sources') {
            // TODO: Re-enable after we have synchronization.
            // agent {
            //     dockerfile {
            ...
            steps {
                dir ('daemon/contrib') {
                    // TODO: Parallelize
                    sh """
                       mkdir -p native && cd native && ../bootstrap
                       make fetch-all TARBALLS=${TARBALLS}
                       """
                }
            }
        }
        ...
        stage('Build packages') {
            steps {
                ...
                +  " DISABLE_CONTRIB_DOWNLOADS=TRUE TARBALLS=${TARBALLS}"
            }
        }
    }
}
