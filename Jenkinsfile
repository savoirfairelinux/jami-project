...
pipeline {
    ...
    environment {
        ...
        CC = 'gcc'                     // some systems do not have 'cc'
        TARBALLS = '/opt/ring-contrib' // sets the cache directory
    }
    ...
    stages {
        ...
        stage('Cache all contrib sources') {
            agent {
                dockerfile {
                    label 'daemon-validation' // until we have contrib synchronization going
                    // TODO: Rename this file to just 'Dockerfile', or
                    // add a minimal variant used just for fetching.
                    // TODO: Setup NFS share to synchronize the tarballs.
                    filename 'daemon/docker/Dockerfile_ubuntu_18.04'
                    args "-e TARBALLS -v ${TARBALLS}:${TARBALLS}"
                    // args '-v /opt/ring-contrib:' + pwd() + '/daemon/contrib/tarballs:rw '
                }
            }
            ...
            steps {
                dir ('daemon/contrib') {
                    // TODO: Parallelize
                    sh 'mkdir -p native && cd native && ../bootstrap && make fetch-all'
                }
            }
        }
        ...
        stage('Build') {
            steps {
                // TODO: Rename to just Makefile.
                // This manages the Dockerfile itself.
                sh 'make -f Makefile.packaging package-ubuntu_20.04 DISABLE_CONTRIB_DOWNLOADS=TRUE'
            }
        }
    }
}
