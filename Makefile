...
$(1)-docker-run-command := docker run \
  ...
  $(3) \
  $$(PACKAGE_$(1)_DOCKER_IMAGE_NAME)

$$($(1)-docker-image-file): docker/Dockerfile-$(1)
	docker build \
	  -t $$($(1)-docker-image-name) \
	  -f docker/Dockerfile-$(1) $(2) $(CURDIR) && \
	touch "$$@"
...
docker/Dockerfile-snap: patches/docker-snap-build-scripts.patch
